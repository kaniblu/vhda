...
class VHDALoss(Loss):
    ...
    def compute(self, batch: BatchData, outputs, step: int = None
        ...
        zconv_post, zgoal_post, zstate_post, zsent_post, zspkr_post = \
            (post[k] for k in ("conv", "goal", "state", "sent", "speaker"))
        zconv_prior, zgoal_prior, zstate_prior, zsent_prior, zspkr_prior = \
            (prior[k] for k in ("conv", "goal", "state", "sent", "speaker"))
        ...
        kld_goal = zgoal_post.kl_div(zgoal_prior).masked_fill(~conv_mask, 0)
        ...
        spkr_mi = \
            (estimate_mi(zspkr_post.view(batch_size * max_conv_len, -1))
             .view(batch_size, max_conv_len).masked_fill(~conv_mask, 0).sum(-1))
        goal_mi = \
            (estimate_mi(zgoal_post.view(batch_size * max_conv_len, -1))
             .view(batch_size, max_conv_len).masked_fill(~conv_mask, 0).sum(-1))
        state_mi = \
        ...
        stats = {
            ...
            "kld-goal": kld_goal.sum(-1).mean(),
            "kld-goal-turn": kld_goal.sum() / conv_lens.sum(),
        ...
        for spkr_idx, spkr in self.vocabs.speaker.i2f.items():
            ...
            spkr_kld_goal = kld_goal.masked_fill(~spkr_mask, 0).sum()
            ...
            spkr_stats = {
                ...
                "kld-goal": spkr_kld_goal / batch_size,
                "kld-goal-turn": spkr_kld_goal / spkr_mask.sum(),
